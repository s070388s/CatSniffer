...
def print_packet_info(fields, decrypted, output_std=None):
    output_std.append_stdout("\n========================= Packet Info =========================")
    output_std.append_stdout(f"Sender: {msb2lsb(fields['sender'].hex())} -> Destination: {msb2lsb(fields['dest'].hex())} PacketID: {msb2lsb(fields['packet_id'].hex())}")
    flags = fields['flags'][0]
    output_std.append_stdout(f"Flags:   {chr(flags)}")
    output_std.append_stdout(f"╰──▶ Hop limit: {(flags >> 5) & 0b111}")
    output_std.append_stdout(f"╰──▶ Want ACK:  {(flags >> 4) & 1}")
    output_std.append_stdout(f"╰──▶ Via MQTT:  {(flags >> 3) & 1}")
    output_std.append_stdout(f"╰──▶ Hop Start: {flags & 0b111}")
    output_std.append_stdout("\n--- Raw Payload (Encrypted) ---")
    output_std.append_stdout(" ".join(f"{b:02X}" for b in fields['payload']))
    output_std.append_stdout("\n--- First Valid Decrypted Protobuf (Hex) ---")
    output_std.append_stdout(" ".join(f"{b:02X}" for b in decrypted))
